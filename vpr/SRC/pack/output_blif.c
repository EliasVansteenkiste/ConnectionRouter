...
void print_logical_block(FILE *fpout, int ilogical_block, t_block *clb) {
	...
	if(logical_block[ilogical_block].type == VPACK_INPAD) {
		...
		if (strcmp(g_atoms_nlist.net[logical_block[ilogical_block].nets->input_nets[0][0]].name,
			...
			if (pb_type->ports[i].type == IN_PORT) {
				...
				*/
				...
				for (int j = 0; j < pb_type->ports[i].num_pins; j++) {
					if (logical_block[ilogical_block].nets->input_nets[0][j] != OPEN) {
						int k;
						for (k = 0; k < pb_type->ports[i].num_pins; k++) {								
							...
							if(a_net_index != OPEN) {
								if(a_net_index == logical_block[ilogical_block].nets->input_nets[0][j]) {
						...
						if(k == pb_type->ports[i].num_pins) {
							...
							vpr_throw(VPR_ERROR_PACK, __FILE__, __LINE__,
								...
								logical_block[ilogical_block].name, g_atoms_nlist.net[logical_block[ilogical_block].nets->input_nets[0][j]].name);
						}
					}
				}
			}
		...
		for (int i = 0; i < pb_type->num_ports; i++) {
			if (pb_type->ports[i].type == OUT_PORT) {
				...
				assert(logical_block[ilogical_block].nets->output_nets[0][0] == pb_route[node_index].atom_net_idx);
		...
		for (int i = 0; i < pb_type->num_ports; i++) {
			if (pb_type->ports[i].type == IN_PORT
				...
				assert(pb_type->ports[i].num_pins == 1);
				assert(logical_block[ilogical_block].nets->input_nets[0][0] != OPEN);
				int node_index =
						pb_graph_node->input_pins[0][0].pin_count_in_cluster;
				fprintf(fpout, "clb_%d_rr_node_%d ", clb_index,	pb_route[node_index].prev_pb_pin_id);
			} else if (pb_type->ports[i].type == OUT_PORT) {
				assert(pb_type->ports[i].num_pins == 1);
				fprintf(fpout, "%s re ", g_atoms_nlist.net[logical_block[ilogical_block].nets->output_nets[0][0]].name);
		...
		for (int i = 0; i < pb_type->num_ports; i++) {
			if (pb_type->ports[i].type == OUT_PORT) {
				...
				assert(pb_type->ports[i].num_pins == 1);
				assert(logical_block[ilogical_block].nets->output_nets[0][0] == pb_route[node_index].atom_net_idx);
				fprintf(fpout, "\n.names %s clb_%d_rr_node_%d\n", g_atoms_nlist.net[logical_block[ilogical_block].nets->output_nets[0][0]].name, clb_index, node_index);
				fprintf(fpout, "1 1\n");
			}
		}
	} else {
			...
			while (port != NULL) {
				if (logical_block[ilogical_block].output_pin_names[port->index] != NULL)	{
					for (int ipin = 0; ipin < port->size; ipin++) {
						int net_index = logical_block[ilogical_block].nets->output_nets[port->index][ipin];
