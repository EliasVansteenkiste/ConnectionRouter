...
t_tnode **tnode_explicit = NULL; /* [0..num_tnodes_explicit - 1] */
int num_tnodes_explicit = 0; /* Number of nodes that do not represent pins but explicit connections */
...
static int is_the_first_explicit_con(int checkBlock, int checkPort, int checkPin, t_nets *curr);
            ...
            case TN_CB_IPIN:
                ...
                for (j = 0; j < ipb_graph_pin->num_output_edges; j++) {
                    ...
                    if (intra_lb_route[dnode].prev_pb_pin_id == i_pin_id) {
                        ...
                        if(i==723) printf("setting tnode loc 1: edge %d (=lookup_tnode_from_pin_id[%d][%d]) \n",count,iblock,dnode);
                ...
                if (timing_inf.timing_analysis_enabled) {
                    ...
                    for (j = 0; j < tnode[i].num_edges; j++) {
                        ...
                        if (intra_lb_route[ipb_graph_pin->pin_timing[j]->pin_count_in_cluster].atom_net_idx
                            ...
                            if(i==723) printf("setting tnode loc 2: edge %d (=lookup_tnode_from_pin_id[%d][%d]) \n",k,tnode[i].block,ipb_graph_pin->pin_timing[j]->pin_count_in_cluster);
            ...
            case TN_CB_OPIN:
                ...
                for (j = 1; j < (int) g_clbs_nlist.net[inet].pins.size(); j++) {
                    ...
                    if (dpin == OPEN) {
                        ...
                        if(i==723) printf("setting tnode loc 3: edge %d \n",j-1);
                        ...
                        if(i==723) printf("setting tnode loc 4: edge %d \n",j-1);
                        tnode[i].out_edges[j - 1].to_node =
...
static void alloc_and_load_tnodes_from_prepacked_netlist(float block_delay,
        ...
        if (logical_block[i].type == VPACK_INPAD) {
            ...
            while (model_port) {
                if (model_port->is_clock == false) {
                    ...
                    j++;
                    num_tnodes += has_multiple_inputs(i);
                    num_tnodes_explicit += has_multiple_inputs(i);
                } else {
                    num_tnodes++;
                }
    ...
    tnode = (t_tnode *) my_calloc(num_tnodes, sizeof (t_tnode));
    num_tnodes_explicit = num_tnodes_explicit << 1;
    tnode_explicit = (t_tnode **) my_calloc(num_tnodes_explicit, sizeof (t_tnode *));
    ...
    inode = 0;
    int inode_explicit = 0;
    for (i = 0; i < num_logical_blocks; i++) {
        //printf("num_logical_blocks %d\n", num_logical_blocks);
        model = logical_block[i].model;
        if (logical_block[i].type == VPACK_INPAD) {
            logical_block[i].nets->output_net_tnodes[0] = (t_tnode **) my_calloc(1, sizeof (t_tnode*));
            logical_block[i].nets->output_net_tnodes[0][0] = &tnode[inode];
            f_net_to_driver_tnode[logical_block[i].nets->output_nets[0][0]] = inode;
            ...
            tnode[inode].num_edges = g_atoms_nlist.net[logical_block[i].nets->output_nets[0][0]].num_sinks();
            ...
            if(i==723) printf("setting tnode loc 5: edge 0 \n");
            ...
            for (cur = logical_block[i].nets; cur != NULL && cur->input_nets != NULL; cur = cur->next) {
            ...
            if(i==723) printf("setting tnode loc 6: edge 0 \n");
            ...
            model_port = model->outputs;
            while (model_port) {
                for (cur = logical_block[i].nets; cur != NULL && cur->output_nets != NULL; cur = cur->next) {
                    ...
                    if (model_port->is_clock == false) {
                        for (k = 0; k < model_port->size; k++) {
                            if (logical_block[i].nets->output_nets[j][k] != OPEN) {
                                ...
                                tnode[inode].num_edges =
                                        g_atoms_nlist.net[cur->output_nets[j][k]].num_sinks();
                                ...
                                if (logical_block[i].clock_net == OPEN) {
                                    ...
                                    tnode[inode + 1].out_edges =
                                            ...
                                            1 * sizeof (t_tedge),
                                            &tedge_ch);
                                    if(i==723) printf("setting tnode loc 7: edge 0 \n");
                                    tnode[inode + 1].out_edges->to_node = inode;
                        ...
                        for (k = 0; k < model_port->size; k++) {
                            if (logical_block[i].nets->output_nets[j][k] != OPEN) {
                                ...
                                f_net_to_driver_tnode[logical_block[i].nets->output_nets[j][k]] = inode;
                                logical_block[i].nets->output_net_tnodes[j][k] = &tnode[inode];
                                //Allocate space for the output edges
                                tnode[inode].num_edges = g_atoms_nlist.net[logical_block[i].nets->output_nets[j][k]].num_sinks();
                                tnode[inode].out_edges = (t_tedge *) my_chunk_malloc(tnode[inode].num_edges * sizeof (t_tedge), &tedge_ch);
                                ...
                                tnode[inode + 1].num_edges = 1;
                                tnode[inode + 1].out_edges = (t_tedge *) my_chunk_malloc(1 * sizeof (t_tedge), &tedge_ch);
                                if(i==723) printf("setting tnode loc 8: edge 0 \n");
                                tnode[inode + 1].out_edges->to_node = inode;
                                tnode[inode + 1].out_edges->Tdel = from_pb_graph_pin->tsu_tco;
            ...
            while (model_port) {
                if (model_port->is_clock == false) {
                    //printf("cur->output_nets, %d\n", logical_block[i].nets);
                    for (cur = logical_block[i].nets; cur != NULL; cur = cur->next) {
                        ...
                        for (k = 0; k < model_port->size; k++) {
                            if (cur->input_nets[j][k] != OPEN) {
                                ...
                                if (logical_block[i].clock_net == OPEN) {
                                    ...
                                    if(inode==721) printf("The type is set here\n");
                                    ...
                                    for (int m = 0; m < from_pb_graph_pin->num_pin_timing; m++) {
                                        ...
                                        if (logical_block[i].nets->output_nets == NULL
                                                || logical_block[i].nets->output_nets[to_pb_graph_pin->port->model_port->index][to_pb_graph_pin->pin_number] == OPEN) {
                                            continue;
                                        }
                                        tnode[inode].out_edges[count].Tdel = from_pb_graph_pin->pin_timing_del_max[m];

                                        tnode[inode].out_edges[count].to_node =
                                                get_tnode_index(logical_block[i].nets->output_net_tnodes[to_pb_graph_pin->port->model_port->index][to_pb_graph_pin->pin_number]);
                                        //tnode[inode].out_edges[count].to_pin = to_pb_graph_pin->pin_number;
                                        //tnode[inode].out_edges[count].to_port = to_pb_graph_pin->port->model_port->index;

                                        //printf("index:%d to_node:%d to_port: %d to_pin: %d", inode, tnode[inode].out_edges[count].to_node, tnode[inode].out_edges[count].to_port, tnode[inode].out_edges[count].to_pin);
                                        count++;
                                    }
                                    ...
                                    tnode[inode].out_edges =
                                            ...
                                            1 * sizeof (t_tedge),
                                            &tedge_ch);
                                    if(i==723) printf("setting tnode loc 9: edge 0 \n");
                                    tnode[inode].out_edges->to_node = inode + 1;
    ...
    for (i = 0; i < num_tnodes; i++) {
        ...
        switch (tnode[i].type) {
            ...
            case TN_CLOCK_OPIN:
                ...
                assert(inet != OPEN);
                //printf("Block: %s <- %d %s %d %d\n", logical_block[tnode[i].block].name, inet, vpack_net[inet].name, vpack_net[inet].num_sinks, tnode[i].num_edges);
                for (j = 1; j < (int) g_atoms_nlist.net[inet].pins.size(); j++) {
                    ...
                    if (g_atoms_nlist.net[inet].is_global) {
                        assert(logical_block[g_atoms_nlist.net[inet].pins[j].block].clock_net == inet);
                        if(i==723) printf("setting tnode loc 10: edge %d \n",j-1);
                        tnode[i].out_edges[j - 1].to_node = get_tnode_index(logical_block[g_atoms_nlist.net[inet].pins[j].block].clock_net_tnode);
                        //printf("Global: to_node:%d\n", tnode[i].out_edges[j - 1].to_node);
                    } else {
                        for (cur = logical_block[g_atoms_nlist.net[inet].pins[j].block].nets; cur != NULL; cur = cur->next) {
//                            if (cur->input_net_tnodes[g_atoms_nlist.net[inet].pins[j].block_port][g_atoms_nlist.net[inet].pins[j].block_port] == NULL) {
//                                if (cur->next == NULL)break;
//                                cur = cur->next;
//                            } else {
                                if(i==723) printf("setting tnode loc 11: edge %d \n",j-1);
                                tnode[i].out_edges[j - 1].to_node = get_tnode_index(cur->input_net_tnodes[g_atoms_nlist.net[inet].pins[j].block_port][g_atoms_nlist.net[inet].pins[j].block_pin]);
//                                tnode[i].out_edges[j - 1].to_port = g_atoms_nlist.net[inet].pins[j].block_port;
//                                tnode[i].out_edges[j - 1].to_pin = g_atoms_nlist.net[inet].pins[j].block_pin;
                                if (cur != logical_block[g_atoms_nlist.net[inet].pins[j].block].nets) {
                                    printf("Set explicit node: %d %d to_port:%d to_pin:%d\n", i, inode_explicit, tnode[i].out_edges[j - 1].to_port, tnode[i].out_edges[j - 1].to_pin);
                                    tnode_explicit[inode_explicit] = &tnode[i];
                                    inode_explicit++;
                                }
                                //printf("Other: to_node:%d to_port:%d to_pin:%d\n",tnode[i].out_edges[j - 1].to_node,tnode[i].out_edges[j - 1].to_port,tnode[i].out_edges[j - 1].to_pin);
                                check = 1;
                                break;
//                            }
                        }
                        if (!check)
                            assert(logical_block[g_atoms_nlist.net[inet].pins[j].block].nets->input_net_tnodes[g_atoms_nlist.net[inet].pins[j].block_port][g_atoms_nlist.net[inet].pins[j].block_pin] != NULL);
                    }
                }
...
static void load_tnode(INP t_pb_graph_pin *pb_graph_pin, INP int iblock,
    ...
    if(i==721) printf("pb_graph_pin is set here\n");
    ...
    if (tnode[i].pb_graph_pin->parent_node->pb_type->blif_model == NULL) {
        ...
        if (tnode[i].pb_graph_pin->type == PB_PIN_INPAD) {
            ...
            tnode[i + 1].out_edges->Tdel = 0;
             if(i==723) printf("setting tnode loc 12: edge 0 \n");
...
static void remove_explicit_connections() {
    int i;
    for (i = 0; i < num_tnodes_explicit; i++) {
        printf("Removed: %d\n", i);
        *tnode_explicit[i] = tnode[num_tnodes - 1];
        num_tnodes--;
    }
    free(tnode_explicit);
}
...
static void set_fanin_for_explicit_connections(int ilevel) {
    ...
    for (i = 0; i < num_at_level; i++) { /* For all the tnodes in the particular level */
        inode = tnodes_at_level[ilevel].list[i];
        if ((inode_explicit = is_explicit_connection(inode)) != -1) { /* This node should have an explicit connection */
            /* If the node has more than one edge that is explicit to another node */
            ...
            if (min_index >= 0) {
                printf("replacing %d with %d\n", min_index, num_tnodes_explicit - 1);
                num_tnodes_explicit--;
                if (min_index == num_tnodes_explicit - 1)
                    continue;
                /* We find the */
                tnode_explicit[min_index] = tnode_explicit[num_tnodes_explicit - 1];
                min_index = -1;
            }
        }
    }
...
void do_constant_net_delay_timing_analysis(t_timing_inf timing_inf,
    ...
    printf("alloc_and_load_timing_graph\n");
    ...
    if (is_prepacked) {
        if (tnode[inode].type == TN_INPAD_SOURCE || tnode[inode].type == TN_INPAD_OPIN ||
            ...
            if (tnode[inode].prepacked_data->model_port_ptr->dir == IN_PORT) {
                inet = logical_block[logic_block].nets->input_nets[iport][ipin];
            } else {
                assert(tnode[inode].prepacked_data->model_port_ptr->dir == OUT_PORT);
                inet = logical_block[logic_block].nets->output_nets[iport][ipin];
            }
...
static void print_primitive_as_blif(FILE *fpout, int iblk, int **lookup_tnode_from_pin_id) {
    ...
    if (logical_block[iblk].type == VPACK_INPAD) {
        ...
        if (logical_block[iblk].pb == NULL) {
            ...
            while (port) {
                ...
                for (j = 0; j < port->size; j++) {
                    if (logical_block[iblk].nets->output_net_tnodes[i][j] != NULL) {
                        node = get_tnode_index(logical_block[iblk].nets->output_net_tnodes[i][j]);
            ...
            assert(
                    pb_route[logical_block[iblk].pb->pb_graph_node->clock_pins[0][0].pin_count_in_cluster].atom_net_idx != OPEN);
            fprintf(fpout, "tnode_%d 0\n\n",
                    lookup_tnode_from_pin_id[iblk][logical_block[iblk].pb->pb_graph_node->clock_pins[0][0].pin_count_in_cluster]);
        }
        assert(node != OPEN);
            ...
            for (i = 0;
                ...
                if (logical_block[iblk].nets->input_nets[0][i] != OPEN) {
                    for (j = 0; j < logical_block[iblk].pb->pb_graph_node->num_input_pins[0]; j++) {
                        if (pb_route[logical_block[iblk].pb->pb_graph_node->input_pins[0][j].pin_count_in_cluster].atom_net_idx
                                != OPEN) {
                            if (pb_route[logical_block[iblk].pb->pb_graph_node->input_pins[0][j].pin_count_in_cluster].atom_net_idx == logical_block[iblk].nets->input_nets[0][i]) {
                                fprintf(fpout, "tnode_%d ",
                                        lookup_tnode_from_pin_id[iblk][logical_block[iblk].pb->pb_graph_node->input_pins[0][j].pin_count_in_cluster]);
                                break;
                            }
                        }
                    }
        ...
        if (logical_block[iblk].pb == NULL) {
            ...
            while (port) {
                for (j = 0; j < port->size; j++) {
                    if (logical_block[iblk].nets->output_net_tnodes[i][j] != NULL) {
                        if (port->size > 1) {
                            fprintf(fpout, "\\\n%s[%d]=%s ", port->name, j,
                                    g_atoms_nlist.net[logical_block[iblk].nets->output_nets[i][j]].name);
                        } else {
                            fprintf(fpout, "\\\n%s=%s ", port->name,
                                    g_atoms_nlist.net[logical_block[iblk].nets->output_nets[i][j]].name);
                        }
                    ...
                    if (pb_route[pb_graph_node->input_pins[i][j].pin_count_in_cluster].atom_net_idx
                        ...
                        if (pb_graph_node->num_input_pins[i] > 1) {
                            fprintf(fpout, "\\\n%s[%d]=tnode_%d ",
                                    ...
                                    j,
                                    lookup_tnode_from_pin_id[iblk][pb_graph_node->input_pins[i][j].pin_count_in_cluster]);
                        } else {
                            fprintf(fpout, "\\\n%s=tnode_%d ",
                                    pb_graph_node->input_pins[i][j].port->name,
                                    lookup_tnode_from_pin_id[iblk][pb_graph_node->input_pins[i][j].pin_count_in_cluster]);
                    ...
                    if (pb_route[pb_graph_node->output_pins[i][j].pin_count_in_cluster].atom_net_idx
                        ...
                        if (pb_graph_node->num_output_pins[i] > 1) {
                            fprintf(fpout, "\\\n%s[%d]=%s ",
                                    ...
                                    j,
                                    g_atoms_nlist.net[pb_route[pb_graph_node->output_pins[i][j].pin_count_in_cluster].atom_net_idx].name);
                        } else {
                            char* port_name =
                                    pb_graph_node->output_pins[i][j].port->name;
                            int pin_count =
                                    pb_graph_node->output_pins[i][j].pin_count_in_cluster;
                            int node_index = pb_route[pin_count].atom_net_idx;
            ...
            for (i = 0; i < pb_graph_node->num_clock_ports; i++) {
                for (j = 0; j < pb_graph_node->num_clock_pins[i]; j++) {
                    if (pb_route[pb_graph_node->clock_pins[i][j].pin_count_in_cluster].atom_net_idx
                            != OPEN) {
                        if (pb_graph_node->num_clock_pins[i] > 1) {
                            fprintf(fpout, "\\\n%s[%d]=tnode_%d ",
                                    pb_graph_node->clock_pins[i][j].port->name,
                                    j,
                                    lookup_tnode_from_pin_id[iblk][pb_graph_node->clock_pins[i][j].pin_count_in_cluster]);
                        } else {
                            fprintf(fpout, "\\\n%s=tnode_%d ",
                                    pb_graph_node->clock_pins[i][j].port->name,
                                    lookup_tnode_from_pin_id[iblk][pb_graph_node->clock_pins[i][j].pin_count_in_cluster]);
            ...
            for (i = 0; i < pb_graph_node->num_output_ports; i++) {
                for (j = 0; j < pb_graph_node->num_output_pins[i]; j++) {
                    if (pb_route[pb_graph_node->output_pins[i][j].pin_count_in_cluster].atom_net_idx
                        ...
                        fprintf(fpout, ".names %s tnode_%d\n",
                                g_atoms_nlist.net[pb_route[pb_graph_node->output_pins[i][j].pin_count_in_cluster].atom_net_idx].name,
                                lookup_tnode_from_pin_id[iblk][pb_graph_node->output_pins[i][j].pin_count_in_cluster]);
                        fprintf(fpout, "1 1\n\n");
                    }
                }
            }
