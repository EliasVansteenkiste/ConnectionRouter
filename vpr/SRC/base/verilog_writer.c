  ...
  for(j=0 ; j < pb->pb_graph_node->num_input_pins[0] ; j++)/*Assuming that LUTs have a single input port*/
  {
     if (logical_block[logical_block_index].nets->input_nets[0][j] != OPEN) 
	  ...
	  for (q = 0; q < pb->pb_graph_node->num_input_pins[0]; q++)
	  {
	    ...
	    if (block[iblock].pb_route[pin_count].atom_net_idx == logical_block[logical_block_index].nets->input_nets[0][j])
	      break;
	  }
...
static conn_list *find_connected_primitives_downhill(int block_num, t_pb *pb, conn_list*list, int **lookup_tnode_from_pin_id)
{
  ...
  for(i=0 ; i < total_output_ports ; i++)
  {
	  ...
	  vpck_net = logical_block[starting_block].nets->output_nets[model_port_index][j]; /*The index for the g_atoms_nlist.net struct corresponding to this source to sink(s) connections*/
